...
# Load pre-processed data to save time and ram
...
res.out <- lapply(speciesNames_range[1:length(speciesNames_range)], function(i) {
  ...
  tryCatch({
    ...
    # write.csv(gbif_occs_fl, file = file.path("SpatialData", "Vector", 
    #                                          "SpeciesOccurrences", 
    #                                          "GBIF", "Flagged", 
    #                                          paste0(i,"_fl.csv")))
    ...
    i <- gsub(" ", "_", i)
    writeRaster(Env_domain, filename = paste0(env_path,"/",i,"/",i, "_env.grd"), 
                overwrite = T)
