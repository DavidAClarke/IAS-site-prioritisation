...
#                path = "E:/SpatialData/Raster/Worldclim")
bio_names <- list.files(path = "C:/Users/david/Documents/PhD/Chapter_3/SpatialData/Raster/Worldclim/wc2-5", 
                        pattern = ".bil$", full.names = T)
bio <- stack(bio_names)
#source("R/02_aus_coast.R") #if not already loaded
...
lapply(spp_list[1:length(spp_list)], function(i) {
  ...
  gbif_occs_shp <- SpatialPointsDataFrame(coords=cbind(gbif_occs$decimalLongitude, 
                                                       gbif_occs$decimalLatitude),
...
#Individual models
my_models <- c("GLM","GAM","MAXENT.Phillips","FDA","GBM") # Algorithms    
...
lapply(spp_list[1:length(spp_list)], function(i){
  ...
  model_opt <- BIOMOD_ModelingOptions(GLM=list(type='quadratic', 
                                               interaction.level=0),
...
lapply(spp_list[1:length(spp_list)], function(i) {
  ...
  model_name <- file.path(global_model_path, i, 
                          paste0(i,".",model_class,".models.out"))
  model_i <- load(model_name)
  
  #Load separate projection models
  model_proj <- file.path(global_model_path, i, paste0("proj_", model_class), 
                          paste0(i,".",model_class,".projection.out"))
  model_pi <- load(model_proj)
  
  # Fit ensemble model
  all_test <- get_evaluations(get(model_i))
  all_tss <- all_test[2, 1, 1:5, 1:4, 1:3]
  all_tss[is.na(all_tss)] <- 0 #if there are any NAs following won't work
...
lapply(spp_list[1:length(spp_list)], function(i) {
  ...
  model_ensem <- file.path(global_model_path, i, 
                           paste0(i,".",model_class,"ensemble.models.out"))
  ...
  write.table(all_ensemble_models_scores, 
              file = file.path(global_model_path, i, 
                               paste0(i,"_eval_ensemble.csv")))
  ...
  write.table(table_importance, 
              file = file.path(global_model_path, i, 
                               paste0(i,"_importance_var.csv")))
...
lapply(spp_list[1:length(spp_list)], function(i) {
  ...
  all_pa <- raster(file.path(global_model_path, i, "proj_global", 
                             "individual_projections", 
                             paste0(i, "_EMcaByTSS_mergedAlgo_mergedRun_mergedData.grd")))
...
lapply(spp_list[1:length(spp_list)], function(i) {
  ...
  #Load separate models
  model_name <- file.path(regional_model_path, i, 
                          paste0(i,".",model_class,".models.out"))
  model_i <- load(model_name)
  
  #Load separate projection models
  model_proj <- file.path(regional_model_path, i, 
                          paste0("proj_", model_class), 
                          paste0(i,".",model_class,".projection.out"))
  model_pi <- load(model_proj)
...
lapply(spp_list[1:length(spp_list)], function(i) {
  ...
  #Load unique ensemble model
  model_ensem <- file.path(regional_model_path, i, 
                           paste0(i,".",model_class,"ensemble.models.out"))
  model_ei <- load(model_ensem)
  ...
  all_ensemble_models_scores <- get_evaluations(get(model_ei))
  write.table(all_ensemble_models_scores, 
              file = file.path(regional_model_path, i, 
                               paste0(i,"_eval_ensemble.csv")))
...
### Results
# Here you decide which results you want the output from. 
...
for(i in spp_list){
  ...
  model_eval <- read.csv(file = file.path(regional_model_path, i, 
                                          paste0(i,"_eval_ensemble.csv")), 
                         sep = " ")
...
table_a <- as.data.frame(cbind(as.character(spp_list),col_n,col_tss,col_sens,
                               col_spec,col_cut))
names(table_a) <- c("Species","n","TSS","Sensitivity","Specificity",
                    "Cut-off binary")

# Write in the directory:
write.table(table_a, 
            file = file.path(regional_model_path, 
                             paste0("Table_",name_model_folder,"_accuracy.txt")),
            row.names = FALSE)
