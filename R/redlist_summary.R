...
IAS_threatend_species <- ggplot(IAS_threats, aes(x = redlistCategory, y = species_per_cat, fill = redlistCategory)) +
  geom_bar(stat = "identity") +
  facet_wrap(~classGroup, nrow = 2) +
  theme_bw() +
  theme(axis.line = element_line(colour = "black"),
...
IAS_threatend_species_stack <- ggplot(IAS_threats_stack, aes(x = classGroup, y = species_per_cat, fill = redlistCategory)) +
