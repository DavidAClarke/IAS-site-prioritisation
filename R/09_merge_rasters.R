...
models <- list.files("/home/dcla0008/nc57_scratch/Zonation/Input_data/maxent_output", 
                     full.names = T)
...
lapply(models[1:length(models)], function(i) {
  ...
  i <- gsub("/home/dcla0008/nc57_scratch/Zonation/Input_data/maxent_output/", 
            "", i)
  i <- gsub("_pred.tif", "", i)
  writeRaster(r_new, filename = paste0(species_path,"/", i, "_pred.tif"), 
              overwrite = T)
  writeRaster(r_new, filename = paste0(species_area_path,"/", i, "_pred.tif"), 
              overwrite = T)
