...
#For testing
variant_names <- c("species_wgt_RAN")
...
create_spp(
  filename = paste0(zonation_path,"/","species_wgt_RAN.spp"),
  weight = wgts, 
  alpha = 1,
  bqp = 1,
  bqp_p = 1,
...
sensitive_site_ID <- create_zproject(name = "species_wgt_RAN_proj", 
                                     dir = zonation_path,
                                     variants = variant_names,
                                     spp_template_dir = species_path,
                                     spp_template_file = paste0(zonation_path,"/", "species_wgt_RAN.spp"),
                                     dat_template_file = paste0(zonation_path,"/", "species_wgt_RAN.dat"),
                                     overwrite = T)
...
write.table(groups_file, 
            file = paste0(zonation_path,"/","species_wgt_RAN_proj","/", "species_groups.txt"), 
            sep = " ", 
            col.names = F,
            row.names = F)
...
sensitive_site_ID <- load_zproject(paste0(zonation_path,"/", "species_wgt_RAN_proj"))
...
save_zvariant(variant, dir = paste0(zonation_path,"/", "species_wgt_RAN_proj"), overwrite = T, debug_msg = F)

#Edit bat file to add use threads
d <- read.delim(file = paste0(zonation_path, "/", "species_wgt_RAN_proj","/",variant@name,".bat"), sep = " ", header = F)
d$V13 <- apply(d, 1, function(x) paste(x, collapse = " "))
d <- d$V13
d <- paste(d, "--use-threads=4")
write.table(d, file = paste0(zonation_path, "/", "species_wgt_RAN_proj","/",variant@name,".bat"), sep = " ", col.names = F, row.names = F, append = F, quote = F)
