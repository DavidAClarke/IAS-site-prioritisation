...
#For testing
variant_names <- c("species_area_wgt_CAZ_KBA")
...
add edge points = 0
memory save mode = 1"
fileConn <- file(paste0(zonation_path,"/","species_area_wgt_CAZ_KBA.dat"))
...
create_spp(
  filename = paste0(zonation_path,"/","species_area_wgt_CAZ_KBA.spp"),
  weight = wgts, 
  alpha = 1,
  bqp = 1,
  bqp_p = 1,
  cellrem = 0.25,
  spp_file_dir = species_area_path, #this will change depending on which features; should match filename
  recursive = FALSE,
...
sensitive_site_ID <- create_zproject(name = "species_area_wgt_CAZ_KBA_proj", 
                                     dir = zonation_path,
                                     variants = variant_names,
                                     spp_template_dir = species_area_path,
                                     spp_template_file = paste0(zonation_path,"/", "species_area_wgt_CAZ_KBA.spp"),
                                     dat_template_file = paste0(zonation_path,"/", "species_area_wgt_CAZ_KBA.dat"),
                                     overwrite = T)
...
write.table(groups_file, 
            file = paste0(zonation_path,"/","species_area_wgt_CAZ_KBA_proj","/", "species_area_groups.txt"), 
            sep = " ", 
            col.names = F,
            row.names = F)
groups_path <- "species_area_groups.txt"

#Load project in to edit variants
sensitive_site_ID <- load_zproject(paste0(zonation_path,"/", "species_area_wgt_CAZ_KBA_proj"))
...
save_zvariant(variant, dir = paste0(zonation_path,"/", "species_area_wgt_CAZ_KBA_proj"), overwrite = T, debug_msg = F)

#Edit bat file to add use threads
d <- read.delim(file = paste0(zonation_path, "/", "species_area_wgt_CAZ_KBA_proj","/",variant@name,".bat"), sep = " ", header = F)
d$V13 <- apply(d, 1, function(x) paste(x, collapse = " "))
d <- d$V13
d <- paste(d, "--use-threads=4")
write.table(d, file = paste0(zonation_path, "/", "species_area_wgt_CAZ_KBA_proj","/",variant@name,".bat"), sep = " ", col.names = F, row.names = F, append = F, quote = F)
