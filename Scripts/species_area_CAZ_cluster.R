...
sensitive_site_ID <- create_zproject(name = "species_area_CAZ_proj", 
                                     ...
                                     spp_template_file = paste0(zonation_path,"/", "species_area_CAZ.spp"),
                                     dat_template_file = paste0(zonation_path,"/", "species_area_CAZ.dat"),
                                     overwrite = T)
...
write.table(groups_file, 
            file = paste0(zonation_path,"/","species_area_CAZ_proj","/", "species_area_groups.txt"), 
            sep = " ", 
            col.names = F,
            row.names = F)
...
sensitive_site_ID <- load_zproject(paste0(zonation_path,"/", "species_area_CAZ_proj"))
...
save_zvariant(variant, dir = paste0(zonation_path,"/", "species_area_CAZ_proj"), overwrite = T, debug_msg = F)

#Edit bat file to add use threads
d <- read.delim(file = paste0(zonation_path, "/", "species_area_CAZ_proj","/",variant@name,".bat"), sep = " ", header = F)
d$V13 <- apply(d, 1, function(x) paste(x, collapse = " "))
d <- d$V13
d <- paste(d, "--use-threads=4")
write.table(d, file = paste0(zonation_path, "/", "species_area_CAZ_proj","/",variant@name,".bat"), sep = " ", col.names = F, row.names = F, append = F, quote = F)
