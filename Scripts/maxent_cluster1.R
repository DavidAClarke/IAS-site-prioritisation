...
#Species
speciesNames_range <- speciesNames_range[1:250] #match respective offset group 
...
mod_evals <- data.frame(Species = character(), 
                        ...
                        MCE = numeric(),
                        Boyce = numeric())
write.csv(mod_evals, file = "/home/dcla0008/nc57_scratch/Maxent/mod_evals1.csv")
...
res.out <- lapply(speciesNames_range[1:length(speciesNames_range)], function(i){
  ...
  tryCatch({
    ...
    write.table(evals, file = "/home/dcla0008/nc57_scratch/Maxent/mod_evals1.csv", append = T, sep = ",", col.names = F)
