...
Coast_shp <- st_read("C:/Users/David Clarke.DESKTOP-NNNNVLL/Dropbox/PhD/Thesis/Data/Chapter_3/SpatialData/Vector/coastal-gshhg-shp-2.3.7/GSHHS_shp/f/GSHHS_f_L1.shp")
Coast_shp <- st_read("C:/Users/dcla0008/Dropbox/PhD/Thesis/Data/Chapter_3/SpatialData/Vector/coastal-gshhg-shp-2.3.7/GSHHS_shp/f/GSHHS_f_L1.shp")
Aus_Coast <- Coast_shp %>% dplyr::filter(id == 6 | id == 32)
Aus_Coast <- st_union(Aus_Coast)
rm(Coast_shp)

pm <- raster(file.path("Zonation", "species_CAZ_proj", "species_CAZ", "species_CAZ_out", "species_CAZ.CAZ_E.rank.compressed.tif"))
...
dist_raster_2 <- projectRaster(dist_raster,pm, res = res(pm), crs = crs(Aus_Coast))

#export the raster
writeRaster(dist_raster_2, file = file.path("SpatialData", "Raster", "dist_aus_coast.tif"), format = "GTiff", overwrite = TRUE)
